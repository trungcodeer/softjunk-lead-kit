...
Latest release status: https://github.com/trungcodeer/softjunk-lead-kit/releases/tag/v2.29.157 targets `5a7f76bb6a0b8cea6a87e676a228624cd13aabad`
...
Current npmjs status from this machine on 2026-05-28: `npm view softjunk-lead-kit --json` returns `E404`, `npm whoami` returns `ENEEDAUTH`, and `npm pack --dry-run --json` passes. Latest release `v2.29.157` records this status. Until an owner publishes with npm auth, use the GitHub release tarball or GitHub package route above; the owner-auth publish template is `NPM_PUBLISH_WORKFLOW.yml.example`.
