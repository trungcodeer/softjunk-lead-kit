...
Current npmjs status from this machine on 2026-05-28: `npm view softjunk-lead-kit --json` returns `E404`, `npm whoami` returns `ENEEDAUTH`, and `npm pack --dry-run --json` passes. Latest release `v2.29.156` records this status. Until an owner publishes with npm auth, use the GitHub release tarball or GitHub package route above; the owner-auth publish template is `NPM_PUBLISH_WORKFLOW.yml.example`.
